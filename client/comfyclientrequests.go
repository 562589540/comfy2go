...
func (c *ComfyClient) GetSystemStats() (*SystemStats, error) {
	...
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/system_stats", c.GetBaseURL()))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetPromptHistoryByID() (map[string]PromptHistoryItem, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/history", c.GetBaseURL()))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetViewMetadata(folder string, file string) (string, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/view_metadata/%s?filename=%s", c.GetBaseURL(), folder, file))
	if err != nil {
		return "", err
	}
...
func (c *ComfyClient) GetImage(image_data DataOutput) (*[]byte, error) {
	...
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/view?%s", c.GetBaseURL(), params.Encode()))
...
func (c *ComfyClient) GetEmbeddings() ([]string, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/embeddings", c.GetBaseURL()))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetQueueExecutionInfo() (*QueueExecInfo, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/prompt", c.GetBaseURL()))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetExtensions() ([]string, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/extensions", c.GetBaseURL()))
	if err != nil {
		return nil, err
	}
...
func (c *ComfyClient) GetObjectInfos() (*graphapi.NodeObjects, error) {
	resp, err := c.httpclient.Get(fmt.Sprintf("%s/object_info", c.GetBaseURL()))
...
func (c *ComfyClient) QueuePrompt(graph *graphapi.Graph) (*QueueItem, error) {
	...
	resp, err := c.httpclient.Post(fmt.Sprintf("%s/prompt", c.GetBaseURL()), "application/json", strings.NewReader(string(data)))
...
func (c *ComfyClient) Interrupt() error {
	resp, err := c.httpclient.Post(fmt.Sprintf("%s/interrupt", c.GetBaseURL()), "application/json", strings.NewReader("{}"))
	if err != nil {
		return err
	}
...
func (c *ComfyClient) EraseHistory() error {
	...
	data := "{\"clear\": \"clear\"}"
	resp, err := c.httpclient.Post(fmt.Sprintf("%s/history", c.GetBaseURL()), "application/json", strings.NewReader(data))
	if err != nil {
		return err
	}
...
func (c *ComfyClient) EraseHistoryItem(promptID string) error {
	...
	resp, err := c.httpclient.Post(fmt.Sprintf("%s/history", c.GetBaseURL()), "application/json", strings.NewReader(item))
	if err != nil {
		return err
	}
